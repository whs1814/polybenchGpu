	...
	DATA_TYPE POLYBENCH_2D(B_outputFromGpu,N,N,n,n), DATA_TYPE POLYBENCH_2D(X_outputFromGpu,N,N,n,n))
{
	...
	for (int t = 0; t < _PB_TSTEPS; t++)
	{
		...
		hipLaunchKernelGGL(adi_kernel1, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(adi_kernel2, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(adi_kernel3, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu);
		hipDeviceSynchronize();
	
		for (int i1 = 1; i1 < _PB_N; i1++)
		{
			hipLaunchKernelGGL(adi_kernel4, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu, i1);
			hipDeviceSynchronize();
		}

		hipLaunchKernelGGL(adi_kernel5, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu);
		hipDeviceSynchronize();
		
		for (int i1 = 0; i1 < _PB_N-2; i1++)
		{
			hipLaunchKernelGGL(adi_kernel6, grid1, block1, 0, 0, n, A_gpu, B_gpu, X_gpu, i1);
			hipDeviceSynchronize();
		}
	}
