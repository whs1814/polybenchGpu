	...
	DATA_TYPE POLYBENCH_2D(D,NI,NL,ni,nl), DATA_TYPE POLYBENCH_2D(D_outputFromGpu,NI,NL,ni,nl))
{
	...
	hipLaunchKernelGGL(mm2_kernel1, grid1, block, 0, 0, ni, nj, nk, nl, alpha, beta, tmp_gpu, A_gpu, B_gpu);
	hipDeviceSynchronize();
	hipLaunchKernelGGL(mm2_kernel2, grid2, block, 0, 0, ni, nj, nk, nl, alpha, beta, tmp_gpu, C_gpu, D_gpu);
	hipDeviceSynchronize();
