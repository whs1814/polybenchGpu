			...
			DATA_TYPE POLYBENCH_2D(symmat_outputFromGpu, M, N, m, n))
{
	...
	hipLaunchKernelGGL(mean_kernel, grid1, block1 , 0, 0, m, n, mean_gpu,data_gpu);
	hipDeviceSynchronize();
	hipLaunchKernelGGL(std_kernel, grid2, block2 , 0, 0, m, n, mean_gpu,stddev_gpu,data_gpu);
	hipDeviceSynchronize();
	hipLaunchKernelGGL(reduce_kernel, grid3, block3 , 0, 0, m, n, mean_gpu,stddev_gpu,data_gpu);
	hipDeviceSynchronize();
	hipLaunchKernelGGL(corr_kernel, grid4, block4 , 0, 0, m, n, symmat_gpu,data_gpu);
	hipDeviceSynchronize();
