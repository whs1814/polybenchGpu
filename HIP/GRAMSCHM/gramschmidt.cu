...
void gramschmidtCuda(int ni, int nj, DATA_TYPE POLYBENCH_2D(A,NI,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(R,NJ,NJ,nj,nj), DATA_TYPE POLYBENCH_2D(Q,NI,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(A_outputFromGpu,NI,NJ,ni,nj))
{
	...
	for (k = 0; k < _PB_NJ; k++)
	{
		hipLaunchKernelGGL(gramschmidt_kernel1, gridKernel1, block, 0, 0, ni, nj, A_gpu, R_gpu, Q_gpu, k);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(gramschmidt_kernel2, gridKernel2, block, 0, 0, ni, nj, A_gpu, R_gpu, Q_gpu, k);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(gramschmidt_kernel3, gridKernel3, block, 0, 0, ni, nj, A_gpu, R_gpu, Q_gpu, k);
		hipDeviceSynchronize();
	}
