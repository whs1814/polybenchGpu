	...
	DATA_TYPE POLYBENCH_2D(ey,NX,NY,nx,ny), DATA_TYPE POLYBENCH_2D(hz,NX,NY,nx,ny), DATA_TYPE POLYBENCH_2D(hz_outputFromGpu,NX,NY,nx,ny))
{
	...
	{
		hipLaunchKernelGGL(fdtd_step1_kernel, grid, block, 0, 0, nx, ny, _fict_gpu, ex_gpu, ey_gpu, hz_gpu, t);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(fdtd_step2_kernel, grid, block, 0, 0, nx, ny, ex_gpu, ey_gpu, hz_gpu, t);
		hipDeviceSynchronize();
		hipLaunchKernelGGL(fdtd_step3_kernel, grid, block, 0, 0, nx, ny, ex_gpu, ey_gpu, hz_gpu, t);
		hipDeviceSynchronize();
	}
